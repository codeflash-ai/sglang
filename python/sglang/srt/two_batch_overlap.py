...
def _split_array_by_balanced_sum(arr: Sequence[int]) -> int:
    ...
    # Precompute the prefix sum for early break optimization
    arr_len = len(arr)
    if arr_len < 2:
        return best_index

    # Instead of recomputing right_sum each iteration, use overall_sum - left_sum
    # Use a memoryview on array-like objects that support the buffer protocol for faster slicing if possible
    # However, since arr can be any Sequence[int], we cannot assume support for buffer protocol, so we stick with direct iteration.

    # The break condition remains as in the original: stop at first local minimum
    for i in range(1, arr_len):
        left_sum += arr[i - 1]
        diff = abs((overall_sum - left_sum) - left_sum)
        if diff <= min_diff:
            min_diff = diff
            best_index = i
        else:
