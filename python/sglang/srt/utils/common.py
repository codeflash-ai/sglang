...
def pytorch_profile(name, func, *args, data_size=-1):
    ...
    # Optimization: Move directory existence check out of inner scope to avoid repeated checks (and possible GIL contention)
    # Ensures the directory is created only once per process lifetime.
    if step_counter == 0:
        os.makedirs("trace", exist_ok=True)

    # File write can be done before profiling to reduce the amount of code in the profiled context.
    # This minimizes extraneous IO in the measured area.
    trace_size_path = f"trace/size_{step_counter}.json"
    with open(trace_size_path, "w") as f:
        json.dump({"size": data_size}, f)

    # Reduce scope under profile context to function call only for best profiling accuracy.
    with profile(
