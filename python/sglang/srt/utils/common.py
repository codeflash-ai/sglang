...
_X86_MACHINES = {"x86_64", "amd64", "i386", "i686"}
...
def is_host_cpu_x86() -> bool:
    ...
    # set lookup is faster than tuple
    # short-circuit hasattr+is_available only if possible
    if machine not in _X86_MACHINES:
        return False
    if not hasattr(torch, "cpu"):
        return False
    return torch.cpu.is_available()


def is_cpu() -> bool:
    # Use os.environ directly to avoid calling getenv machinery if the key doesn't exist
    # This is only a very minor optimization, but saves a function call in the common case
    return os.environ.get("SGLANG_USE_CPU_ENGINE", "0") == "1" and is_host_cpu_x86()
