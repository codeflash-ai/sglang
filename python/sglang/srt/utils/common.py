...
def decode_video_base64(video_base64):
    ...
    if frame_format == "PNG":
        # Optimize PNG signature scan by using bytes.find and advance efficiently
        signature = b'\x89PNG\r\n\x1a\n'
        sig_len = len(signature)
        search_start = 0
        bytes_len = len(video_bytes)
        while True:
            idx = video_bytes.find(signature, search_start)
            if idx == -1:
                break
            img_starts.append(idx)
            search_start = idx + sig_len
    else:
        # For JPEG, scan efficiently for 0xFFD8
        soi = b'\xff\xd8'
        soi_len = len(soi)
        search_start = 0
        bytes_len = len(video_bytes)
        while True:
            idx = video_bytes.find(soi, search_start)
            if idx == -1:
                break
            img_starts.append(idx)
            search_start = idx + soi_len

    # Precompute all start/end pairs to avoid repeated index lookups

    frames = []
    num_imgs = len(img_starts)
    if num_imgs == 0:
        return np.array([]), (0, 0)
    img_ends = img_starts[1:] + [len(video_bytes)]
    # Use local vars for frequently used calls outside loop for efficiency
    append_frame = frames.append
    stack = np.stack
    # Use PIL.Image to convert directly
    for start_idx, end_idx in zip(img_starts, img_ends):
    ...
    return stack(frames, axis=0), img.size
