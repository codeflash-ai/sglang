...
class BumpAllocator:
    def __init__(self, buffer_size: int, dtype, device):
        ...
        self._buffer_len = buffer_size  # Avoid repeated len() call

    def allocate(self, size: int):
        # Locally cache pointer, avoid redundant attribute access
        pointer = self._pointer
        end_ptr = pointer + size

        # Use precomputed _buffer_len to avoid repeated len() call
        assert end_ptr <= self._buffer_len
        # Slicing avoids copies, but use indices directly for faster access
        output = self._buffer[pointer : end_ptr]
        self._pointer = end_ptr
        return output
