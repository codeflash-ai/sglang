...
def get_num_new_pages(
    ...
    if prefix_lens is None or decode:
        ...
        assert decode
        # Use torch.count_nonzero, which is typically faster than .int().sum() for boolean masks
        return torch.count_nonzero(seq_lens % page_size == 1).item()


    assert prefix_lens.device == cpu_device
    # Avoid unnecessary creation of intermediate tensors by using integer division directly
    num_pages_after = (seq_lens.add(page_size - 1)).div(page_size, rounding_mode='floor')
    num_pages_before = (prefix_lens.add(page_size - 1)).div(page_size, rounding_mode='floor')
    # Use in-place subtraction to save memory
    num_new_pages = num_pages_after.sub(num_pages_before)
    sum_num_new_pages = num_new_pages.sum(dtype=torch.int64)
