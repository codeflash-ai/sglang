...
def lru_cache_frozenset(maxsize=128):
    def _to_hashable(o):
        # Fast path for built-in hashables
        ...
        try:
            ...
            pass
        # Fast path for tuple/list/Sequence (skip str/bytes again)
        if isinstance(o, (tuple, list)):
            return tuple(_to_hashable(v) for v in o)
        elif isinstance(o, dict):
            # Use sorted items for determinism and efficiency
            return frozenset(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Use tuple instead of frozenset for kwargs for faster lookup
            # Also using sorted for deterministic key ordering
            if kwargs:
                h_kwargs_tuple = tuple(
                    sorted((k, _to_hashable(v)) for k, v in kwargs.items())
                )
            else:
                h_kwargs_tuple = ()
            key = (h_args, h_kwargs_tuple)
            try:
                val = cache[key]
