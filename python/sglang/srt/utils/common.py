...
def get_device_sm_nvidia_smi():
    try:
        ...
        # Use subprocess.check_output for slightly lower overhead, and to avoid extra object creation.
        output = subprocess.check_output(
            ["nvidia-smi", "--query-gpu=compute_cap", "--format=csv,noheader"],
            text=True,
        )
        # Get the first line of output (assuming at least one GPU exists)
        # Avoid split("\n")[0]: use partition for efficiency and clarity.
        compute_cap_str = output.strip().partition('\n')[0]
