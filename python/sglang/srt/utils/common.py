...
def get_bool_env_var(name: str, default: str = "false") -> bool:
    ...
    if value_lower in _TRUTHY_VALUES:
        return True
    if value_lower in _FALSY_VALUES:
        return False
    # warning branch, only when not in known values
    if value_lower not in _warned_bool_env_var_keys:
        logger.warning(
            f"get_bool_env_var({name}) see non-understandable value={value_lower} and treat as false"
        )
        _warned_bool_env_var_keys.add(value_lower)
    return False
...
def is_flashinfer_available():
    ...
    if not get_bool_env_var("SGLANG_IS_FLASHINFER_AVAILABLE", default="true"):
        return False
    # importlib.util.find_spec can be slow, but cannot cache aggressively in this code due to possible dynamic installs
