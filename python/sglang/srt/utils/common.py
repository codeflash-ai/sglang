...
def is_sm100_supported(device=None) -> bool:
    # Short-circuit before accessing any CUDA/HIP specifics if not a supported device
    if not (
        torch.cuda.is_available() and torch.version.cuda or torch.version.hip is not None
    ):
        return False
    cap = torch.cuda.get_device_capability(device)
    # Use tuple compare for slight efficiency, otherwise preserve original logic
    if cap[0] == 10 and torch.version.cuda >= "12.8":
        return True
    return False
...
# Inline and specialize is_cuda and is_hip from srt.utils.common for faster access
def _is_hip() -> bool:
    # Avoid extra indirection from external imports within hot paths
    return torch.version.hip is not None
