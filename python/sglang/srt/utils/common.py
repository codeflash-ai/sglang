...
def get_local_ip_by_nic(interface: str = None) -> Optional[str]:
    ...
    try:
        ...
        af_inet = addresses.get(netifaces.AF_INET)
        if af_inet:
            for addr_info in af_inet:
                ip = addr_info.get("addr")
                if ip and ip != "127.0.0.1" and ip != "0.0.0.0":
                    return ip

        af_inet6 = addresses.get(netifaces.AF_INET6)
        if af_inet6:
            for addr_info in af_inet6:
                ip = addr_info.get("addr")
...
def get_local_ip_auto(fallback: str = None) -> str:
    ...
    # Use os.environ.get instead of os.getenv for faster access, and only attempt the second if missing
    host_ip = os.environ.get("SGLANG_HOST_IP")
    if host_ip is None:
