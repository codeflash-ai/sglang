...
def get_physical_cpus_by_numa():
    ...
    cpus_allowed_list = set(psutil.Process().cpu_affinity())  # Convert to set once for fast intersection
    ...
    for node, core_to_cpu in physical_by_node.items():
        cpus = core_to_cpu.values()
        # Collect only allowed CPUs; no need to create a sorted list until the very end
        allowed_cpus = set(cpus) & cpus_allowed_list
