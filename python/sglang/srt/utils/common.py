...
def configure_ipv6(dist_init_addr):
    ...
    # Only validate the address without brackets.
    # Use the cached validation function to reduce overhead significantly.
    stripped_host = host[1:end]
    if not is_valid_ipv6_address(stripped_host):
        raise ValueError(f"invalid IPv6 address: {host}")

    port_str = None
    addr_len = len(addr)
    next_idx = end + 1
    if addr_len > next_idx:
        if addr[next_idx] == ":":
            # slice assignment is fast for strings, no need to optimize further
            port_str = addr[next_idx + 1 :]
        else:
