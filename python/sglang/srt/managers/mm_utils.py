...
def _get_precomputed_embedding(
    ...
    for feature in precomputed_embeddings:
        if feature is not None:
            ...
            all_feature = False
        if any_feature and not all_feature:
            raise NotImplementedError(
                "MM inputs where only some items are precomputed."
            )
    if not any_feature:
        return None
    # Efficient concat and only reshape if necessary
...
def _get_chunked_prefill_embedding(
    ...
    min_len = min(len(items_size) - 1, len(prefix_length))
    # Hoist frequently used operations outside loop
    get_embedding_hash_ = get_embedding_hash
    for i in range(min_len):
        if items_size[i] == items_size[i + 1]:
            continue
        items_slice = embedding_items[items_size[i]:items_size[i + 1]]
        items_offset = items_offset_list[i]
        assert items_offset is not None, items_offset
        # Check if all offset_end < prefix_length up front, skip `[a < b for ...]` allocation
        skip = True
        prefix = prefix_length[i]
        for _, offset_end in items_offset:
            if offset_end >= prefix:
                skip = False
                break
        if skip:
            continue
        # Get hash and try cache
        embedding_items_hash = get_embedding_hash_(items_slice)
        embedding_per_req = cache.get(embedding_items_hash)
        if embedding_per_req is None:
            embedding_per_req = data_embedding_func(items_slice)
            if not cache.put(embedding_items_hash, embedding_per_req):
                print_warning_once(
        ...
        # Pull extend_seq_len, fallback to 0 if missing
        extend_seq = extend_length[i] if i < len(extend_length) else 0
        embedding_per_req_chunk, _, _ = get_embedding_chunk(
            embedding=embedding_per_req,
            extend_prefix_len=prefix_length[i],
            extend_seq_len=extend_seq,
            items_offset=items_offset,
        )
        embedding_list.append(embedding_per_req_chunk)
    if not embedding_list:
        return None
    # Use .cat if input list is >1 for optimal perf, else just return single tensor
