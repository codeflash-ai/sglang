...
class EnvField:
    ...
    def get(self) -> Any:
        # Use local bindings and direct getenv to minimize attribute lookups
        env_name = self.name
        set_to_none = self._set_to_none
        value = os.environ.get(env_name) if not set_to_none else None
        ...
        try:
            ...
            warnings.warn(
                f'Invalid value for {env_name}: {e}, using default "{self.default}"'
            )
            return self.default
    ...
    def get_set_value_or(self, or_value: Any):
        ...
        # Inline is_set() to minimize overhead for hot paths
        env_name = self.name
        if env_name in os.environ or self._set_to_none:
            return self.get()
        return or_value
