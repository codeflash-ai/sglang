...
class NemoConvSubsampling(torch.nn.Module):
    ...
    def __init__(
        ...
        in_channels = 1
        # Preallocate list for improved locality and append speed when possible
        layers = []
        if subsampling == "dw_striding":
            ...
            first_conv_kwargs = {
                ...
                "stride": self._stride,
                "padding": None if self.is_causal else self._left_padding,
            }
            layers.append(first_conv_class(**first_conv_kwargs))
            in_channels = conv_channels
            layers.append(activation)
            for i in range(self._sampling_num - 1):
                # depthwise conv
                dw_conv_class = CausalConv2D if self.is_causal else torch.nn.Conv2d
                dw_conv_kwargs = {
                    "in_channels": in_channels,
                ...
                layers.append(dw_conv_class(**dw_conv_kwargs))
                # 1x1 Pointwise conv
                layers.append(
                    torch.nn.Conv2d(
                        in_channels=in_channels,
            ...
            # Use loop-local variables for faster instantiation in the hot path
            conv_class = CausalConv2D if self.is_causal else torch.nn.Conv2d
            ...
            conv_class = CausalConv1D if self.is_causal else torch.nn.Conv1d
            for i in range(self._sampling_num):
            ...
            self._left_padding = left_right_pad
            self._right_padding = left_right_pad
            # First depthwise, then pointwise
            layers.append(
                torch.nn.Conv1d(
                    in_channels=in_channels,
                    out_channels=in_channels,
            ...
            first_out_ch = feat_out if self._sampling_num == 1 else conv_channels
            layers.append(
                torch.nn.Conv1d(
                    in_channels=in_channels,
            ...
            in_channels = conv_channels
            layers.append(activation)
            # Reuse arguments where possible for tight loop
            for i in range(self._sampling_num - 1):
                layers.append(
                    torch.nn.Conv1d(
                        in_channels=in_channels,
                ...
                out_ch = feat_out if self._sampling_num == i + 2 else conv_channels
                layers.append(
                    torch.nn.Conv1d(
                        in_channels=in_channels,
                        out_channels=out_ch,
        ...
        if subsampling in ["dw_striding", "striding"]:
            ...
            # Use factory for potential faster layer creation
    ...
    def conv_split_by_batch(self, x):
        ...
        if self.subsampling_conv_chunking_factor > 1:
            ...
            # Use integer arithmetic when possible for efficiency
            x_ceil = 2**31 / (self._conv_channels * (self._stride ** 2))
            num_elem = torch.numel(x)
            p = math.ceil(math.log(num_elem / x_ceil, 2))
            cf = 2**p
        new_batch_size = b // cf
        if new_batch_size == 0:
            return (x, False)
        # Use a list comprehension for slightly improved performance,
        # and preallocate the needed list
        chunks = torch.split(x, new_batch_size, 0)
        outputs = [self.conv(chunk) for chunk in chunks]
        return (
