...
class FeedForward(nn.Module):
    ...
    def __init__(
        self, d_model, d_inner, dropout_rate, activation="sigmoid", bias_in_glu=True
    ):
        super().__init__()
...
class NemoConvSubsampling(torch.nn.Module):
    ...
    @codeflash_capture(
        function_name="NemoConvSubsampling.__init__",
        tmp_dir_path="/tmp/codeflash_prg_d4a_/test_return_values",
        tests_root="/home/ubuntu/work/repo/test",
        is_fto=True,
    )
    def __init__(
        self,
        ...
        if subsampling == "dw_striding":
            ...
            if self.is_causal:
                ...
                left_right_pad = (self._kernel_size - 1) // 2
                self._left_padding = left_right_pad
                self._right_padding = left_right_pad
            ...
            if self.is_causal:
                ...
                self._max_cache_len = 0
            for i in range(self._sampling_num):
                out_ch = feat_out if self._sampling_num == i + 1 else conv_channels
                if self.is_causal:
                    layers.append(
                        CausalConv1D(
                            in_channels=in_channels,
            ...
            for i in range(self._sampling_num - 1):
                ...
                layers.append(
                    torch.nn.Conv1d(
                        in_channels=in_channels,
                        out_channels=(
                            feat_out if self._sampling_num == i + 2 else conv_channels
                        ),
                        kernel_size=1,
                        stride=1,
                        padding=0,
        ...
        if subsampling in ["dw_striding", "striding"]:
            ...
            final_out_length = int(out_length.item())
            self.out = torch.nn.Linear(conv_channels * final_out_length, feat_out)
    ...
    def reset_parameters(self):
        if self._subsampling == "dw_striding":
            with torch.no_grad():
                ...
                fc_scale = (self._feat_out * self._feat_in / self._sampling_num) ** (
                    -0.5
                )
    ...
    def conv_split_by_channel(self, x):
        ...
        results"""
        # Prepare the conv/activation layers only once to avoid repeated indexed lookups
        conv = self.conv
        x = conv[0](x)
        x = conv[1](x)

        _sampling_num = self._sampling_num
        conv_chunking_factor = self.subsampling_conv_chunking_factor

        for i in range(_sampling_num - 1):
            (_, c, t, _) = x.size()
            if conv_chunking_factor > 1:
                cf = conv_chunking_factor
            else:
                # Avoid repeated computation and pow-in-log
                numel = x.numel()
                q = numel // (2**31)
                # avoid log(0) if numel is really small (all dims==1)
                if q == 0:
                    p = 0
                else:
                    # Use bit_length for integer log2, faster than math.log for ints
                    p = (q).bit_length()
                cf = 2**p

            # min-val guards: can't have chunk size 0, can't cover all channels/time with zero
            # Avoid casting to int via // if already int
            ...
            # Channel chunked conv2d
            x = self.channel_chunked_conv(conv[conv2idx], new_c, x)

            # Instead of list comprehension followed by torch.cat, preallocate list and avoid per-loop attribute lookup
            next_conv = conv[conv2idx + 1]
            if x.shape[2] <= new_t:
                # avoid splitting if not needed; saves memory/cycle
                x_conv = next_conv(x)
                x = x_conv
            else:
                # This branch covers the common case
                chunked = torch.split(x, new_t, 2)
                res = [next_conv(chunk) for chunk in chunked]
                x = torch.cat(res, 2)
            x = conv[conv2idx + 2](x)
        return x
    ...
    def channel_chunked_conv(self, conv, chunk_size, x):
        ...
        # Optimize split/concatenation, avoid list append where possible
        chunks = tuple(torch.split(x, chunk_size, 1))
        out_chunks = [None] * len(chunks)
        ind = 0
        ...
        # Bulk variables to minimize repeated attr lookups
        w = conv.weight
        b = conv.bias
