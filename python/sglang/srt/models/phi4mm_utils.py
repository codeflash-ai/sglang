...
def calc_length(lengths, all_paddings, kernel_size, stride, ceil_mode, repeat_num=1):
    ...
    # Convert to float once, outside the loop, to avoid repeated conversions
    lengths = lengths.to(torch.float)

    # For repeat_num > 1, iteratively apply the calculation
    for _ in range(repeat_num):
        # Use in-place operations to reduce memory allocations
        lengths = lengths.add(add_pad).div_(stride).add_(one)
        if ceil_mode:
            lengths = torch.ceil(lengths)
        else:
            lengths = torch.floor(lengths)
    # Final conversion back to int (no in-place to avoid overwriting input type)
