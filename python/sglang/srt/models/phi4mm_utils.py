...
class T5RelativeAttentionLogitBias(nn.Module):
    ...
    def forward(self, x):
        maxpos = x.size(1)
        # Use arange with size directly, for performance
        device = x.device
        dtype = torch.long
        ctx_range = torch.arange(maxpos, device=device, dtype=dtype).unsqueeze(1)
        mem_range = torch.arange(maxpos, device=device, dtype=dtype).unsqueeze(0)
        # Vectorized subtraction for relative position
        relative_position = mem_range - ctx_range

        # Clamp the relative positions in a single step (faster than masked_fill)
        relative_position = relative_position.clamp(-self.max_distance, self.max_distance - 1)
    ...
    def _bucket_relative_position(self, relative_position):
        ...
        # Avoid log(0) for large positions:
        rel_pos_float = relative_position.float().clamp(min=1)
        relative_position_if_large = max_exact + (
            torch.log(rel_pos_float / max_exact)
