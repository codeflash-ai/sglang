...
def split_tensor_along_last_dim(
    ...
    last_dim = tensor.ndim - 1  # Faster than .dim()
    size = tensor.shape[last_dim]  # Faster than .size()[]
    last_dim_size = divide(size, num_partitions)
    # Split.
    # Split.
...
def get_pp_indices(
    ...
    if partition_list_str is not None:
        ...
        if n_partitions != pp_size:
            raise ValueError(f"{n_partitions= } does not match {pp_size=}.")
        if total_layers != num_hidden_layers:
            raise ValueError(f"{total_layers= } does not match {num_hidden_layers=}.")
        # Use a local variable for start_layer to avoid re-summing in case of large partition arrays
        if pp_rank == 0:
            start_layer = 0
        else:
